...
(define (enqueue x q)
  (cond ((> (add1 (queue-size-r q)) (queue-size-l q)) (match q
                                                         [(queue left right size-l size-r)
                                                          (struct-copy queue q [left (rotate left (cons x right) empty-stream)]
                                                                       [right null]
                                                                       [size-r 0]
                                                                       [size-l (stream-length (rotate left right empty-stream))])]))        
...
(define (dequeue q)
   (if (> (queue-size-r q) (sub1 (queue-size-l q))) (match q
                                                         [(queue left right size-l size-r)
                                                          (struct-copy queue q [left (rotate (stream-rest left) right empty-stream)]
                                                                       [right null]
                                                                       [size-r 0]
                                                                       [size-l (stream-length (rotate (stream-rest left) right empty-stream))])])        
        (if (stream-empty? (stream-rest (queue-left q)))
             (match q
               [(queue left right size-l size-r)
                (struct-copy queue q [left empty-stream]
                             [right null]
                             [size-r 0]
                             [size-l (stream-length (rotate (stream-rest left) right empty-stream))])])
